...
const Products = () => {
  ...
  const cartPush = (product) => {
    ...
    if (existingProduct) {
      ...
      // If the product isn't in the cart, add it with the current quantity
      setCartItems([...cartItems, { ...product, quantity }]);
    }
  
    // Reset temporary quantity after adding to cart
  ...
  useEffect(() => {
    getCartItems();
    getFavourites();
  }, []); // Only rerun when uniqueId changes
  
  useEffect(() => {
    postCartItems();
  }, [cartItems]); // Only rerun when cartItems changes
    ...
    <div className="md:grid grid-cols-12 mt-4 z-12 h-auto flex flex-col">
      ...
      <div className="min-w-54 left_container col-span-4 flex flex-col items-center z-1 bg-white sticky top-0">
        ...
        <ul className="text-center p-2 grid grid-cols-2 gap-2 md:grid-cols-none w-1/2">
          <li
            className={`py-2 rounded-full border border-gray-600 font-semibold mb-2 cursor-pointer hover:bg-gray-10 ${
          ...
          <li
            className={`py-2 rounded-full border border-gray-600 font-semibold mb-2 cursor-pointer hover:bg-gray-10 ${
              category === "women" && "bg-gray-100"
            }`}
            onClick={() => {
              setFilteredProducts(
                productList.filter((product) => product.category === "women")
              );
              setSearchValue("");
              setCategory("women");
            }}
          ...
          <li
            className={`py-2 rounded-full border border-gray-600 font-semibold mb-2 cursor-pointer hover:bg-gray-10 ${
              category === "kids" && "bg-gray-100"
            }`}
            onClick={() => {
              setFilteredProducts(
                productList.filter((product) => product.category === "kids")
              );
              setSearchValue("");
              setCategory("kids");
            }}
        ...
        <div className="flex flex-wrap gap-6 justify-center items-start lg:justify-start">
          {filteredProducts.length > 0 ? (
            filteredProducts.map((product, index) => (
              <div
                key={index}
                className="w-34 bg-white rounded-2xl overflow-hidden items-start"
              >
                    ...
                    <div className="flex justify-between items-center px-4 py-2 rounded-lg">
                      <div className="flex gap-2 items-center">
                        ...
                        <span className="text-gray-600">
                          {temporaryQuantities[product.id] || 1}
                        </span>
                        <span
                          className="px-2 py-1 bg-gray-100 rounded cursor-pointer hover:bg-gray-200"
            ...
            <p className="sm:text-lg md:text-4xl font-bold text-center w-full">
              No products found
            </p>
          )}
        </div>
